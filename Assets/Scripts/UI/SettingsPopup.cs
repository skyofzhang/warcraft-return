// UI-09 设置（弹窗版）：用于战斗暂停菜单进入设置
// 依据：UI策划案 v3.2（设置项：音量/画质/帧率/语言）
using UnityEngine;
...
public class SettingsPopup : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        prt.sizeDelta = new Vector2(880f, 980f);
        ...
        var safe = CreateSafeArea(prt);
        if (safe == null) return;

        CreateText(safe, "_Text_Title", "设置", new Vector2(0.5f, 0.94f), new Vector2(500f, 70f), 42, TextAnchor.MiddleCenter);

        CreateText(safe, "_Text_BGM", "音乐音量", new Vector2(0.30f, 0.84f), new Vector2(240f, 40f), 26, TextAnchor.MiddleLeft);
        bgmSlider = CreateSlider(safe, new Vector2(0.52f, 0.78f), new Vector2(540f, 44f));

        CreateText(safe, "_Text_SFX", "音效音量", new Vector2(0.30f, 0.70f), new Vector2(240f, 40f), 26, TextAnchor.MiddleLeft);
        sfxSlider = CreateSlider(safe, new Vector2(0.52f, 0.64f), new Vector2(540f, 44f));

        CreateText(safe, "_Text_Quality", "画质", new Vector2(0.30f, 0.56f), new Vector2(240f, 40f), 26, TextAnchor.MiddleLeft);
        qualityGroup = CreateToggleGroup(safe, "_TG_Quality");
        qLow = CreateToggleButton(safe, qualityGroup, "_Toggle_Quality_Low", "低", new Vector2(0.42f, 0.50f), new Vector2(160f, 80f));
        qMed = CreateToggleButton(safe, qualityGroup, "_Toggle_Quality_Medium", "中", new Vector2(0.60f, 0.50f), new Vector2(160f, 80f));
        qHigh = CreateToggleButton(safe, qualityGroup, "_Toggle_Quality_High", "高", new Vector2(0.78f, 0.50f), new Vector2(160f, 80f));

        CreateText(safe, "_Text_FPS", "帧率", new Vector2(0.30f, 0.42f), new Vector2(240f, 40f), 26, TextAnchor.MiddleLeft);
        fpsGroup = CreateToggleGroup(safe, "_TG_FPS");
        fps30 = CreateToggleButton(safe, fpsGroup, "_Toggle_FPS_30", "30", new Vector2(0.55f, 0.36f), new Vector2(180f, 80f));
        fps60 = CreateToggleButton(safe, fpsGroup, "_Toggle_FPS_60", "60", new Vector2(0.75f, 0.36f), new Vector2(180f, 80f));

        CreateText(safe, "_Text_Language", "语言", new Vector2(0.30f, 0.28f), new Vector2(240f, 40f), 26, TextAnchor.MiddleLeft);
        langGroup = CreateToggleGroup(safe, "_TG_Language");
        langCn = CreateToggleButton(safe, langGroup, "_Toggle_Language_CN", "简体中文", new Vector2(0.55f, 0.22f), new Vector2(260f, 80f));
        langEn = CreateToggleButton(safe, langGroup, "_Toggle_Language_EN", "English", new Vector2(0.80f, 0.22f), new Vector2(220f, 80f));

        hintText = CreateText(safe, "_Text_Hint", "", new Vector2(0.5f, 0.14f), new Vector2(800f, 40f), 22, TextAnchor.MiddleCenter);
        ...
        if (bgmSlider != null) bgmSlider.onValueChanged.AddListener(v => { curBgm = v; ApplyRuntime(false); });
        if (sfxSlider != null) sfxSlider.onValueChanged.AddListener(v => { curSfx = v; ApplyRuntime(false); });
    ...
    private static RectTransform CreateSafeArea(RectTransform parent)
    {
        ...
        rt.offsetMin = new Vector2(40f, 60f);
        rt.offsetMax = new Vector2(-40f, -60f);
        return rt;
    }
    ...
    private static Toggle CreateToggleButton(RectTransform parent, ToggleGroup group, string name, string label, Vector2 anchorPos, Vector2 size)
    {
        ...
        if (btn != null && t != null) btn.onClick.AddListener(() => t.isOn = true);
        return t;
    }
